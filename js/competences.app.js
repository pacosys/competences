...
var recherche = function(evt){
	...
	var target = document.querySelector(input.getAttribute("search-target"));
	//console.log(target);
	var text = input.value;
	searchArray  = noAccent2LowerCase(text);
	searchArray  = searchArray.split(' ');
	var pageBody = document.body;
	var elements = target.querySelectorAll(input.getAttribute("elements")||"div");
	var bool=true;
	var alt=true;
	var erase=true;
	...
	if(input.hasAttribute("numtarget"))
		document.querySelector(input.getAttribute("numtarget")).textContent = number;
}
...
var sortElementsIn = function(element, sortTagAttribute) {
...
var app = angular.module('competences',[]);
	...
	//Students and competences
		...
		this.selectStudent = function(cs){
			...
			this.newStudent = (cs.id?true:false);
			//this.backupCurrentStudent = this.newStudent?{}:cs.
			this.setPopup("student");
		};
		this.setNewStudent = function(){
			var id = 1;
			while(Students[id])id++;
			this.selectStudent(Students[id]={"eid":id});
		};
		this.closeNewStudent = function(){
		}
